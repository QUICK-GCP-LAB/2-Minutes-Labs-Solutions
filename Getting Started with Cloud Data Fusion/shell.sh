...
create_data_fusion_instance() {
    ...
    # Create the Cloud Data Fusion instance
    while true; do
        echo "Creating Cloud Data Fusion instance..."
        gcloud beta data-fusion instances create "$instance_name" \
            --location="$location" \
            --edition=basic \
            --enable_stackdriver_logging \
            --enable_stackdriver_monitoring \
            --impersonate-service-account="$PROJECT_NUMBER-compute@developer.gserviceaccount.com"

        # Check if the command was successful
        if [ $? -eq 0 ]; then
            echo "Cloud Data Fusion instance '$instance_name' creation initiated."
            
            # Wait until the instance appears in the describe command
            while true; do
                echo "Checking if instance '$instance_name' is created..."
                if gcloud beta data-fusion instances describe "$instance_name" --location="$location" > /dev/null 2>&1; then
                    echo "Cloud Data Fusion instance '$instance_name' created successfully."
                    break 2  # Break out of both loops
                else
                    echo "Instance '$instance_name' not found. Waiting for 30 seconds..."
                    sleep 30
                fi
            done
            break
        else
            echo "Failed to create Cloud Data Fusion instance. Retrying in 2 Minutes..."
            sleep 120
        fi
    done
}
...
echo "${BLUE}${BOLD}Click Here: $INSTANCE_URL${RESET}"
