...
echo "${CYAN}${BOLD}[Step 1] Enabling Cloud Run API...${RESET}"
gcloud services enable run.googleapis.com

# Step 2: Clone the repository
echo "${GREEN}${BOLD}[Step 2] Cloning Google Cloud generative AI repository...${RESET}"
git clone https://github.com/GoogleCloudPlatform/generative-ai.git

# Step 3: Navigate to the required directory
echo "${YELLOW}${BOLD}[Step 3] Navigating to the 'gemini-streamlit-cloudrun' directory...${RESET}"
cd generative-ai/gemini/sample-apps/gemini-streamlit-cloudrun

# Step 4: Copy chef.py from the cloud storage bucket
echo "${MAGENTA}${BOLD}[Step 4] Copying 'chef.py' from Google Cloud Storage...${RESET}"
gsutil cp gs://spls/gsp517/chef.py .

# Step 5: Remove unnecessary files
echo "${BLUE}${BOLD}[Step 5] Removing existing files: Dockerfile, chef.py, requirements.txt...${RESET}"
...
echo "${YELLOW}${BOLD}[Step 9] Setting up Python virtual environment...${RESET}"
python3 -m venv gemini-streamlit
source gemini-streamlit/bin/activate
python3 -m  pip install -r requirements.txt

# Step 10: Start Streamlit application
echo "${MAGENTA}${BOLD}[Step 10] Running Streamlit application in the background...${RESET}"
...
echo "${BLUE}${BOLD}[Step 11] Creating Artifact Registry repository...${RESET}"
AR_REPO='chef-repo'
SERVICE_NAME='chef-streamlit-app' 
gcloud artifacts repositories create "$AR_REPO" --location="$GCP_REGION" --repository-format=Docker

# Step 12: Submit Cloud Build
echo "${RED}${BOLD}[Step 12] Submitting Cloud Build...${RESET}"
gcloud builds submit --tag "$GCP_REGION-docker.pkg.dev/$GCP_PROJECT/$AR_REPO/$SERVICE_NAME"

# Step 13: Deploy Cloud Run Service
echo "${CYAN}${BOLD}[Step 13] Deploying Cloud Run service...${RESET}"
...
echo "${GREEN}${BOLD}[Step 14] Fetching Cloud Run service URL...${RESET}"
