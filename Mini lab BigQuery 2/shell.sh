...
# Step 1: Set the PROJECT_ID and REGION variables
echo "${BOLD}${BLUE}Fetching the project ID and region${RESET}"
export PROJECT_ID=$(gcloud config get-value project)
export REGION=$(gcloud compute project-info describe --format="value(commonInstanceMetadata.items[google-compute-default-region])")

# Step 2: Create BigQuery connection
echo "${BOLD}${YELLOW}Creating BigQuery connection${RESET}"
...
# Step 3: Query external Spanner database
echo "${BOLD}${MAGENTA}Querying external Spanner database${RESET}"
...
echo "${BOLD}${CYAN}Creating BigQuery view 'ecommerce.order_history'${RESET}"
