...
# Step 5: Copy sample images from the training bucket to your new bucket
echo "${BLUE}${BOLD}Copying Sample Images to Your Bucket${RESET}"
gsutil -m cp gs://cloud-training/gsp897/cosmetic-test-data/*.png \
gs://${PROJECT_ID}/cosmetic-test-data/

# Step 6: List image files and save to a CSV file
echo "${MAGENTA}${BOLD}Listing Image Files and Saving to CSV${RESET}"
gsutil ls gs://${PROJECT_ID}/cosmetic-test-data/*.png > /tmp/demo_cosmetic_images.csv

# Step 7: Copy the CSV file to your Cloud Storage bucket
echo "${GREEN}${BOLD}Copying CSV File to Cloud Storage${RESET}"
gsutil cp /tmp/demo_cosmetic_images.csv gs://${PROJECT_ID}/demo_cosmetic_images.csv

echo

# Step 8: Display link for creating a dataset in Visual Inspection
echo "${CYAN}${BOLD}Go to this link to create a Visual Inspection dataset:${RESET}""https://console.cloud.google.com/ai/visual-inspection/datasets/create?project=$PROJECT_ID"
